...
# RAPH: import focal loss for loss function
from openfold.utils.my_functions import FocalLoss
from openfold.utils.my_functions import f1_loss
...
# RAPH: 
def subst_rate_loss(subst_rate_pred, label_seq, anchor_seq, mask_indels, seq_length_crop, **kwargs,):
    batch_size = label_seq.size()[0]
    seq_len = label_seq.size()[1]
    mask_crop = torch.zeros((batch_size, seq_len)).to(mask_indels.device)
    for i in range(batch_size):
        mask_crop[i,:seq_length_crop[i]] = torch.ones((1,seq_length_crop[i].item()))
    merged_mask = torch.logical_and(mask_crop.bool(), mask_indels.bool())

    seq_truth  = [torch.masked_select(label_seq[i], merged_mask[i]) for i in range(batch_size)]
    seq_anchor = [torch.masked_select(anchor_seq[i], merged_mask[i]) for i in range(batch_size)]

    subst_rates = []
    for k in range(len(seq_truth)):
        if seq_truth[k].size()[0] > 0:
            subst_rate_k = sum([seq_truth[k][i].item() != seq_anchor[k][i].item() and seq_truth[k][i].item()!=21 and seq_anchor[k][i].item()!=21 for i in range(seq_truth[k].size()[0])]) / seq_truth[k].size()[0]
        else:
            subst_rate_k = 0.1
        subst_rates.append(subst_rate_k)
    
    true_subst_rates = torch.tensor([subst_rates]).to(subst_rate_pred.device)
    
    loss_f = torch.nn.L1Loss()
    loss = loss_f(true_subst_rates, subst_rate_pred)
    return loss


def subst_rate_loss_pos(subst_rate_pred, subst_pos_pred, mask_indels, seq_length_crop, **kwargs,):
    batch_size = subst_rate_pred.size()[0]
    seq_len = subst_pos_pred.size()[1]
    mask_crop = torch.zeros((batch_size, seq_len)).to(mask_indels.device)
    for i in range(batch_size):
        mask_crop[i,:seq_length_crop[i]] = torch.ones((1,seq_length_crop[i].item()))
    merged_mask = torch.logical_and(mask_crop.bool(), mask_indels.bool())
    merged_mask = merged_mask[:,:,None].expand(-1,-1,2)
    
    # TODO : un loss qui compare la prédiction du taux de substitution et le taux de substitution impliqué par la prédiction des positions
    #subst_pos_pred = [subst_pos_pred[i,mask_indels[i]] for i in range(subst_pos_pred.size()[0])]
    subst_pos_pred = [torch.masked_select(subst_pos_pred[i], merged_mask[i]).view(-1,2) for i in range(batch_size)]
    
    subst_rate_pred_noGrad = subst_rate_pred.detach()    # remove grad so we don't modify the subst rate head
    sm = nn.Softmax(dim=1)    # we might wanna modify this with a soft argmax
    eps = 0.1    # to force the softmax to be SHARP and make the softmax close to an argmax
    subst_pos_pred = [sm(subst_pos_pred[i] / eps) for i in range(len(subst_pos_pred))]

    #mean_subst_pos_pred = torch.tensor([torch.mean(subst_pos_pred[i][:,1], dim=0) for i in range(len(subst_pos_pred))]).to(subst_pos_pred[0].device)
    mean_subst_pos_pred = torch.tensor([torch.mean(subst_pos_pred[i][:,1]-subst_pos_pred[i][:,0], dim=0) for i in range(len(subst_pos_pred))]).to(subst_pos_pred[0].device)
    loss_f = torch.nn.L1Loss()
    loss = loss_f(mean_subst_pos_pred, subst_rate_pred_noGrad[:,0])
    return loss


def fscore_loss_pos(subst_pos_pred, label_seq, aatype, anchor_seq, mask_substitutions_ancestor, mask_indels, seq_length_crop, residue_index, **kwargs,):
    batch_size = subst_pos_pred.size()[0]
    seq_len = subst_pos_pred.size()[1]
    mask_crop = torch.zeros((batch_size, seq_len)).to(mask_indels.device)
    for i in range(batch_size):
        mask_crop[i,:seq_length_crop[i]] = torch.ones((1,seq_length_crop[i].item()))
    merged_mask = torch.logical_and(mask_crop.bool(), mask_indels.bool())
    merged_mask_wide = merged_mask[:,:,None].expand(-1,-1,2)
    
    sm = nn.Softmax(dim=1)    # we might wanna modify this with a soft argmax
    subst_pos_pred  = [torch.masked_select(sm(subst_pos_pred[i])[:,0], merged_mask[i]) for i in range(batch_size)]
    subst_pos_truth = [torch.masked_select(mask_substitutions_ancestor[i], merged_mask[i]) for i in range(batch_size)]
    losses = []
    for i in range(batch_size):
        losses.append(f1_loss(subst_pos_pred[i], subst_pos_truth[i]))
    #losses = f1_loss(subst_pos_pred, subst_pos_truth)
    loss = torch.mean(torch.tensor(losses))
    #loss = loss*0
    return loss


# RAPH: My positional loss that compares true sequence with predicted sequence, only regarding the positions of the substitutions
def position_seq_crossentropy(subst_pos_pred, label_seq, anchor_seq, mask_substitutions_ancestor, mask_indels, seq_length_crop, residue_index, **kwargs,):
    batch_size = subst_pos_pred.size()[0]
    seq_len = subst_pos_pred.size()[1]
    mask_crop = torch.zeros((batch_size, seq_len)).to(mask_indels.device)
    for i in range(batch_size):
    ...
    subst_pos_pred = subst_pos_pred.transpose(-1,-2)
    
    weights = torch.FloatTensor([1,7]).to(subst_pos_pred[0].device)    # RAPH: the weight is a hyperparameter we must ajust
    loss_f_ce = nn.CrossEntropyLoss(weight = weights, reduction='none')
    errors = loss_f_ce(subst_pos_pred, mask_substitutions_ancestor)
    errors = errors * merged_mask.int().float()    # cast from bool to float and mask the positions with indels
    loss = torch.sum(errors) / torch.sum(merged_mask)
    #print('position loss', loss)
    return loss


# RAPH: My loss that compares true sequence with predicted sequence 
def seq_crossentropy(seq_pred, label_seq, anchor_seq, mask_indels, mask_substitutions_ancestor, seq_length_crop, residue_index, **kwargs,):
    batch_size = label_seq.size()[0]
    seq_len = label_seq.size()[1]
    mask_crop = torch.zeros((batch_size, seq_len)).to(mask_indels.device)
    for i in range(batch_size):
        mask_crop[i,:seq_length_crop[i]] = torch.ones((1,seq_length_crop[i].item()))
    merged_mask = torch.logical_and(mask_crop.bool(), mask_indels.bool())
    merged_mask_subst = torch.logical_and(merged_mask.bool(), mask_substitutions_ancestor.bool())
    
    seq_pred = seq_pred.transpose(-1,-2)
    #seq_truth = torch.nn.functional.one_hot(label_seq.long(), num_classes=23).float().transpose(-1,-2)
    seq_truth = label_seq.long()
    loss_f = nn.CrossEntropyLoss(reduction='none')
    errors = loss_f(seq_pred, seq_truth)
    errors = errors * merged_mask_subst.int().float()
    loss = torch.sum(errors) / torch.sum(merged_mask_subst)
    return loss



# RAPH: Computation of other non-loss metrics, such as sequence similarity and confusion matrices
def metrics_computation_batch(cerberus_predictions, subst_rate, label_seq, ancestor_seq, anchor_seq, mouse_seq, human_seq, mask_substitutions_ancestor, merged_mask, msa_feat, **kwargs,):
    subst_rate_pred, subst_pos_pred, seq_pred = cerberus_predictions
    batch_size = subst_rate_pred.size()[0]
    all_metrics = []
    for i in range(batch_size):
        all_metrics.append(metrics_computation_single_positions(seq_pred[i], subst_pos_pred[i], subst_rate_pred[i], subst_rate[i], label_seq[i], ancestor_seq[i], anchor_seq[i], mouse_seq[i], human_seq[i], mask_substitutions_ancestor[i], merged_mask[i], msa_feat[i]))
    return all_metrics


def metrics_computation_single_positions(seq_pred, subst_pos_pred, subst_rate_pred, subst_rate, label_seq, ancestor_seq, anchor_seq, mouse_seq, human_seq, mask_substitutions_ancestor, merged_mask, msa):
    #seq_len = label_seq.size()[0]
    #mask_crop = torch.zeros((seq_len)).to(mask_indels.device)
    #mask_crop[:seq_length_crop] = torch.ones((seq_length_crop.item()))
    #merged_mask = torch.logical_and(mask_crop.bool(), mask_indels.bool())
    merged_mask_wide = merged_mask[:,None].expand(-1,2)
    merged_mask_extrawide = merged_mask[:,None].expand(-1,22)

    s_pred_raw   = torch.masked_select(seq_pred, merged_mask_extrawide).view(-1,22).tolist()
    s_pred_aa_f  = torch.masked_select(torch.argmax(seq_pred, dim=1), merged_mask).tolist()
    
    s_pred_pos_f = torch.masked_select(torch.argmax(subst_pos_pred, dim=1), merged_mask).tolist()
    s_pred_pos_raw = torch.masked_select(subst_pos_pred, merged_mask_wide).view(-1,2).tolist()

    s_truth_f  = torch.masked_select(label_seq, merged_mask).tolist()
    s_anchor_f = torch.masked_select(anchor_seq, merged_mask).tolist()
    s_mouse_f = torch.masked_select(mouse_seq, merged_mask).tolist()
    s_human_f  = torch.masked_select(human_seq, merged_mask).tolist()
    s_ancestor_f  = torch.masked_select(ancestor_seq, merged_mask).tolist()
    mask_substitutions_ancestor_f = torch.masked_select(mask_substitutions_ancestor, merged_mask).tolist()
    m_len = len(s_ancestor_f)

    s_msa_f = []
    for a in msa:
        a_aa = residue_constants.onehot_to_str_sequence(np.array(a[:,:22].cpu()))
        a_str = ''
        for i, aa in enumerate(a_aa):
            if merged_mask[i].item()==1:
                a_str += aa
        s_msa_f.append(a_str)
    
    if m_len == 0:
        print('len of s_anchor_f is 0, returning empty dict for metrics')
        return {}
    
    # metrics for positions only
    tp = sum([s_pred_pos_f[i]==1 and mask_substitutions_ancestor_f[i]==1 for i in range(m_len)])
    tn = sum([s_pred_pos_f[i]==0 and mask_substitutions_ancestor_f[i]==0 for i in range(m_len)])
    fp = sum([s_pred_pos_f[i]==1 and mask_substitutions_ancestor_f[i]==0 for i in range(m_len)])
    fn = sum([s_pred_pos_f[i]==0 and mask_substitutions_ancestor_f[i]==1 for i in range(m_len)])

    metrics = {}
    
    metrics['mask_substitutions_ancestor'] = mask_substitutions_ancestor_f

    # metrics about substitution rates
    metrics['substitution_rate_pred'] = subst_rate_pred.item()
    metrics['substitution_rate_truth'] = subst_rate.item()

    # sequences
    metrics['seq_pred'] = s_pred_aa_f
    metrics['seq_truth'] = s_truth_f
    metrics['seq_ancestor'] = s_ancestor_f
    metrics['seq_anchor'] = s_anchor_f
    metrics['seq_mouse'] = s_mouse_f
    metrics['seq_human'] = s_human_f
    metrics['msa'] = s_msa_f

    # metrics about positions of substitutions
    metrics['seq_pred_pos'] = s_pred_pos_f
    metrics['tp'] = tp
    metrics['fp'] = fp
    metrics['tn'] = tn
    metrics['fn'] = fn
    if 2*tp+fp+fn>0.:
        metrics['pos_0_f1_score'] = 2*tp/(2*tp+fp+fn)
    #metrics['seq_length'] = int(seq_length.item())
    return metrics
...
class AlphaFoldLoss(nn.Module):
    ...
    def forward(self, out, batch, _return_breakdown=False):
        loss_fns = {
            # RAPH: add our own losses functions
            #"subst_rate_loss": lambda: subst_rate_loss(    # L1 loss for the substitution rate
            #    subst_rate_pred=out["cerberus_predictions"][0],
            #    **{**batch, **self.config.distogram},
            #),
            #"subst_rate_loss_pos": lambda: subst_rate_loss_pos(    # L1 loss for subst rate for the positions of substitutions
            #    subst_rate_pred=out["cerberus_predictions"][0],
            #    subst_pos_pred=out["cerberus_predictions"][1],
            #    **{**batch, **self.config.distogram},
            #),
            #"fscore_loss": lambda: fscore_loss_pos(    # fscore loss for positions of substitutions
            #    subst_pos_pred=out["cerberus_predictions"][1],
            #    **{**batch, **self.config.distogram},
            #),
            "position_seq_crossentropy": lambda: position_seq_crossentropy(    # crossentropy loss for prediction of positions of substitutions
                subst_pos_pred=out["cerberus_predictions"][1],
                subst_rate_pred=out["cerberus_predictions"][0],
                **{**batch, **self.config.distogram},
            ),
            #"seq_crossentropy": lambda: seq_crossentropy(    # crossentropy loss for sequence prediction
            #    seq_pred=out["cerberus_predictions"][2],
            #    **{**batch, **self.config.distogram},
            #),
        }
        ...
        for loss_name, loss_fn in loss_fns.items():
            ...
            if(torch.isnan(loss) or torch.isinf(loss)):
                ...
                #m_d = residue_constants.ID_TO_HHBLITS_AA
                #label_aa = ''.join([m_d[v.item()] for v in batch['label_seq'][0]])
                
                #print(label_aa)
                print(out["cerberus_predictions"][0])
                #print(out["cerberus_predictions"][1])
        ...
        crop_len = torch.mean(batch["seq_length_crop"].float())
        seq_len = torch.tensor(batch["aatype"].shape[-1])
        ...
        if(not _return_breakdown):
            return cum_loss
        metrics = metrics_computation_batch(out["cerberus_predictions"], **{**batch, **self.config.distogram},)    # returns a list (long as batch size) of metrics
        return cum_loss, losses, metrics
