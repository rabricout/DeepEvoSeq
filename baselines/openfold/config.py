...
import json    # RAPH: for the hyper_config
...
def model_config(
    ...
    nature_only=False,    # RAPH
    hyper_config=None,    # RAPH: the configuration file for hyperparameters, if wanted
    strat='', 
):
    c = copy.deepcopy(config)
    c.model.nature_only = nature_only    #RAPH: to learn either all or only the nature of substitutions for given positions
    c.model.baselines_module.strat = strat
    # TRAINING PRESETS
    ...
    # RAPH: MY PRESET
        ...
        #c.data.train.max_extra_msa = 512
    ...
    # RAPH: modify the hyperparameters
    if hyper_config is not None:
        with open(hyper_config, 'r') as hyper_config_file:
            hyper_config_dict = json.load(hyper_config_file)
        for k in hyper_config_dict.keys():
            if k == 'nb_evoformer_layers':
                c.model.evoformer_mini_stack.no_blocks = hyper_config_dict[k]
            if k == 'subst_rate_loss_pos_weight':
                c.loss.subst_rate_loss_pos.weight = hyper_config_dict[k]
            if k == 'activation':
                #c.model.subst_rate_head.activation = hyper_config_dict[k]
                c.model.subst_pos_head.activation = hyper_config_dict[k]
                c.model.seq_head.activation = hyper_config_dict[k]
            if k == 'norm':
                c.model.subst_rate_head.norm = hyper_config_dict[k]
                c.model.subst_pos_head.norm = hyper_config_dict[k]
                c.model.seq_head.norm = hyper_config_dict[k]
            if k == 'intern_dim_1':
                c.model.subst_rate_head.intern_dim_1 = hyper_config_dict[k]
                c.model.subst_pos_head.intern_dim_1 = hyper_config_dict[k]
                c.model.seq_head.intern_dim_1 = hyper_config_dict[k]
            if k == 'intern_dim_2':
                c.model.subst_pos_head.intern_dim_2 = hyper_config_dict[k]
                c.model.seq_head.intern_dim_2 = hyper_config_dict[k]
...
max_len = mlc.FieldReference(768, field_type=int) #512
...
config = mlc.ConfigDict(
    {
        "data": {
            "common": {
                ...
                "max_recycling_iters": 3,  # RAPH: default value is 3
            ...
            "eval": {
                "fixed_size": True,
                "subsample_templates": False,  # We want top templates.
                #"masked_msa_replace_fraction": 0.0,    #0.15,
        ...
        "model": {
            ...
            "evoformer_mini_stack": {    # RAPH: custom mini evoformer
                "c_m": c_m,
                "c_z": c_z,
                "c_hidden_msa_att": 32,
                "c_hidden_opm": 32,
            ...
            "baselines_module": {    # RAPH: adding parameters for the baselines strategies
                "strat": 'mouse',
            },
            "subst_rate_head": {    # RAPH: adding parameters for the custom head
                "c_m": c_m,
                "max_len": max_len,
                "transition_n": 1,
                "msa_dim": 128,
            ...
            "subst_pos_head": {    # RAPH: adding parameters for the custom head
                "c_m": c_m,
                "max_len": max_len,
                "transition_n": 1,
                "msa_dim": 128,
            ...
            "seq_head": {    # RAPH: adding parameters for the custom head
                "c_m": c_m,
                "max_len": max_len,
                "transition_n": 1,
                "msa_dim": 128,
