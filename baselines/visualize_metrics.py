...
def seq_crossentropy(json_dicts):
    all_crossentropies = []
    for i, m_dict in enumerate(json_dicts):
        if 'seq_truth' in m_dict.keys() and 'seq_pred' in m_dict.keys():
            seq_pred_raw = torch.tensor(m_dict['seq_pred_raw'])[None,:,:].transpose(-1,-2)
            seq_truth = torch.tensor(m_dict['seq_truth'])[None,:].long()
            loss = torch.nn.CrossEntropyLoss()
            all_crossentropies.append(loss(seq_pred_raw, seq_truth).item())
    plt.plot(all_crossentropies)
    plt.show()
    return
...
def plot_subst_rate_diff(json_dicts):
    ...
    for m_dict in json_dicts:
        ...
        if m_dict['substitution_rate_truth_CM'] > 0.5:
            print('seqs')
            s_t = ''.join([ID_TO_HHBLITS_AA[a] for a in m_dict['seq_truth']])
            s_a = ''.join([ID_TO_HHBLITS_AA[a] for a in m_dict['seq_anchor']])
    ...
    plt.plot(subst_truth, label='truth subst rate CM (castor/marmotta)', c='red')
    plt.plot(subst_pred, label='predicted subst rate CM (castor/marmotta)', c='blue')
    plt.plot(subst_proxy, label='proxy of subst rate CM (castor/marmotta)', c='green')
    plt.plot(subst_human, label='truth subst rate HM (human/marmotta)', linestyle='--', c='black')
    plt.legend(loc='upper left')
...
while not leave:
    ...
    for i, k in enumerate(keys):
        print(i, 'for', k)
    print('x1 for confusion matrix between truth and prediction')
    print('x2 for confusion matrix between truth and prediction on substitution positions')
    print('x3 for position of substitutions (excluding epoch 0)')
    print('x4 for position confusion matrix')
    print('x5 for F-score (position head)')
    print('x5b for F-score (sequence head)')
    print('x6 for precision-recall curve')
    print('x7 for ROC curve')
    print('x8 for difference between substitution rate truth and predicted')
    print('x9 for correlation between subst_rate CM (castor/marmotte) and subst_rate HM (humain/marmotte)')
    print('x10 for subst_rates')
    print('x11 for comparison between true subst rate and subst rate in seq predictions')
    print('x12 for subst rates in cloud (pred vs truth)')
    print('x13 for seq crossentropy loss (computed locally)')
    ...
    if m_input == 'exit':
        ...
        continue
    try:
        if m_input not in ['x1', 'x2', 'x3', 'x4', 'x5', 'x5b', 'x6', 'x7', 'x8', 'x9', 'x10', 'x11', 'x12', 'x13']:
            x = int(m_input)
            if x>len(keys)-1:
    ...
    if m_input == 'x1':
        ...
        print('seq crossentropy')
        seq_crossentropy(json_dicts)
