...
def plot_subst_rate_diff(json_dicts):
    ...
    subst_proxy = []
    for m_dict in json_dicts:
        if 'substitution_rate_truth_CA' in m_dict.keys() and 'substitution_rate_pred_CA' in m_dict.keys():
            subst_diff_pred.append(np.abs(m_dict['substitution_rate_truth_CA'] - m_dict['substitution_rate_pred_CA']))
            subst_diff_proxy.append(np.abs(m_dict['substitution_rate_truth_CA'] - m_dict['substitution_rate_truth_HA']/0.045*0.027))
            if m_dict['substitution_rate_truth_CA'] > 0.5:
                print('seqs')
            ...
            subst_pred.append(m_dict['substitution_rate_pred_CA'])
            subst_truth.append(m_dict['substitution_rate_truth_CA'])
            subst_human.append(m_dict['substitution_rate_truth_HA'])
            subst_proxy.append(m_dict['substitution_rate_truth_HA']/0.045*0.027)
    ...
    plt.plot(subst_truth, label='truth subst rate CA (beaver/squirrel)', c='red')
    plt.plot(subst_pred, label='predicted subst rate CA (beaver/squirrel)', c='blue')
    plt.plot(subst_proxy, label='proxy of subst rate CA (beaver/squirrel)', c='green')
    plt.plot(subst_human, label='truth subst rate HA (beaver/squirrel)', linestyle='--', c='black')
    plt.legend(loc='upper left')
...
def plot_subst_rates(json_dicts):
    subst_rate_CA = []
    subst_rate_HA = []
    subst_rate_CA_noIndels = []
    subst_rate_HA_noIndels = []
    subst_rate_CA_pred = []
    for m_dict in json_dicts:
        if 'substitution_rate_truth_CA' in m_dict.keys() and 'substitution_rate_pred_CA' in m_dict.keys():
            subst_rate_CA.append(m_dict['substitution_rate_truth_CA'])
            subst_rate_HA.append(m_dict['substitution_rate_truth_HA'])
            subst_rate_CA_noIndels.append(m_dict['substitution_rate_truth_CA_noIndels'])
            subst_rate_HA_noIndels.append(m_dict['substitution_rate_truth_HA_noIndels'])
            subst_rate_CA_pred.append(m_dict['substitution_rate_pred_CA'])
    print(sum(subst_rate_CA_noIndels)/len(subst_rate_CA_noIndels))
    plt.plot(subst_rate_CA, label='subst_rate_CA')
    plt.plot(subst_rate_HA, label='subst_rate_HA')
    plt.plot(subst_rate_CA_noIndels, label='subst_rate_CA_noIndels')
    plt.plot(subst_rate_HA_noIndels, label='subst_rate_HA_noIndels')
    plt.plot(subst_rate_CA_pred, label='subst_rate_pred_CA')
    plt.legend(loc='upper left')
...
def pos_subst_rate(json_dicts):
    subst_rate_CA = []
    subst_rate_CA_pos_pred = []
    epoch = 1
    subst = 0
    tot = 0
    local_true_subst_rate = []
    for m_dict in json_dicts:
        if 'substitution_rate_truth_CA' in m_dict.keys() and 'tp' in m_dict.keys():
            if epoch == m_dict['epoch_nb']:
                subst_rate_CA_pos_pred.append(subst/tot)
                subst_rate_CA.append(np.mean(np.array(local_true_subst_rate)))
                subst = 0
                tot = 0
                epoch += 1
            local_true_subst_rate.append(m_dict['substitution_rate_truth_CA'])
            subst += m_dict['tp']+m_dict['fp']
            tot += m_dict['tp']+m_dict['fp']+m_dict['tn']+m_dict['fn']
    subst_rate_CA_pos_pred.append(subst/tot)
    subst_rate_CA.append(np.mean(np.array(local_true_subst_rate)))
    plt.plot(subst_rate_CA, label='subst_rate_CA')
    plt.plot(subst_rate_CA_pos_pred, label='subst_rate_pred_pos_CA')
    plt.legend(loc='upper left')
    plt.title('substitution_rates')
    plt.show()
...
def print_correlations(json_dicts):
    ...
    for m_dict in last_1000_dicts:
        if 'substitution_rate_truth_CA' in m_dict.keys() and 'substitution_rate_pred_CA' in m_dict.keys():
            subst_CA.append(m_dict['substitution_rate_truth_CA'])
            subst_HA.append(m_dict['substitution_rate_truth_HA'])
            subst_CA_pred.append(m_dict['substitution_rate_pred_CA'])
    subst_CA = np.array(subst_CA)
    subst_HA = np.array(subst_HA)
    subst_CA_pred = np.array(subst_CA_pred)
    #pearson_correlation_truth = scipy.stats.pearsonr(subst_CA, subst_HA)
    #pearson_correlation_pred  = scipy.stats.pearsonr(subst_CA_pred, subst_HA)
    pearson_correlation_CA_pred = scipy.stats.pearsonr(subst_CA_pred, subst_CA)
    # defining proxy
    mean_CA = np.linalg.norm(subst_CA, ord=1) / len(subst_CA)
    mean_HA = np.linalg.norm(subst_HA, ord=1) / len(subst_HA)
    #print(mean_HA, 0.11)
    #print(mean_CA, 0.083)
    subst_CA_proxy = subst_HA /0.045*0.027 #mean_HA * mean_CA
    pearson_correlation_CA_proxy = scipy.stats.pearsonr(subst_CA_proxy, subst_CA)
    
    diff_pred_truth_CA_L1  = np.linalg.norm((subst_CA-subst_CA_pred), ord=1) / len(subst_CA-subst_CA_pred)
    diff_proxy_truth_CA_L1 = np.linalg.norm((subst_CA-subst_CA_proxy), ord=1) / len(subst_CA-subst_CA_proxy)
    diff_pred_truth_CA_L2  = np.linalg.norm((subst_CA-subst_CA_pred), ord=2) / len(subst_CA-subst_CA_pred)
    diff_proxy_truth_CA_L2 = np.linalg.norm((subst_CA-subst_CA_proxy), ord=2) / len(subst_CA-subst_CA_proxy)

    #print('pearson_correlation between CA and HA', pearson_correlation_truth)
    #print('pearson_correlation between CA predicted and HA', pearson_correlation_pred)
    print('pearson_correlation between CA predicted and CA truth', pearson_correlation_CA_pred)
    print('pearson_correlation between CA proxy and CA truth', pearson_correlation_CA_proxy)
    print('mean distance (L1 norm) between CA pred and CA truth', diff_pred_truth_CA_L1)
    print('mean distance (L1 norm) between CA proxy and CA truth', diff_proxy_truth_CA_L1)
    print('mean distance (L2 norm) between CA pred and CA truth', diff_pred_truth_CA_L2)
    print('mean distance (L2 norm) between CA proxy and CA truth', diff_proxy_truth_CA_L2)
    input()
...
def subst_rates_scatter(json_dicts):
    ...
    for m_dict in json_dicts:
        if 'substitution_rate_truth_CA' in m_dict.keys() and 'substitution_rate_pred_CA' in m_dict.keys():
            subst_rate.append(m_dict['substitution_rate_truth_CA'])
            subst_rate_pred.append(m_dict['substitution_rate_pred_CA'])
            subst_HA.append(m_dict['substitution_rate_truth_HA'])
    subst_rate_proxy = np.array(subst_HA) / 0.045*0.027 #mean_HA * mean_CA
...
while not leave:
    ...
    print('x9 (substRate) for correlation between subst_rate CA (castor/ancestor) and subst_rate HA (humain/marmotte)')
