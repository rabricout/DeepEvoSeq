...
NUM_WORKERS=4
if is_simple:
    train_dataset = DeepEvoSeqSimpleDatasetNature(label_to_idx, data_path+'train_all.pkl', data_path+'train_microgale.pkl', window=window)
    eval_dataset = DeepEvoSeqSimpleDatasetNature(label_to_idx, data_path+'eval_all.pkl', data_path+'eval_microgale.pkl', window=window)
else:
    train_dataset = DeepEvoSeqDatasetNature(label_to_idx, data_path+'train_all.pkl', data_path+'train_microgale.pkl', window=window)
    eval_dataset = DeepEvoSeqDatasetNature(label_to_idx, data_path+'eval_all.pkl', data_path+'eval_microgale.pkl', window=window)
nb_amino_acids = 21
nb_species = 5


# Prepare model based on attention and is_simple
model_deepEvoSeq = DeepEvoSeqGeneric(nb_species, nb_amino_acids, attention=use_attention, is_simple=is_simple, internal_dim=48)
...
writer = SummaryWriter("runs_nature_microgale/"+run_id+date_str)
with open("runs_nature_microgale/specs_"+run_id+date_str+'.txt', 'w', encoding='utf-8') as f:    # save model specs and training hyperparameters
...
np.save('runs_nature_microgale/'+run_id+date_str+'/nature_eval_values.npy', data_best_5_epochs)