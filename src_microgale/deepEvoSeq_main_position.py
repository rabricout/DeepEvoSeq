...
NUM_WORKERS=4
if is_simple:
    train_dataset = DeepEvoSeqSimpleDatasetPosition(label_to_idx, data_path+'train_all.pkl', data_path+'train_microgale.pkl')
    eval_dataset = DeepEvoSeqSimpleDatasetPosition(label_to_idx, data_path+'eval_all.pkl', data_path+'eval_microgale.pkl')
else:
    train_dataset = DeepEvoSeqDatasetPosition(label_to_idx, data_path+'train_all.pkl', data_path+'train_microgale.pkl')
    eval_dataset = DeepEvoSeqDatasetPosition(label_to_idx, data_path+'eval_all.pkl', data_path+'eval_microgale.pkl')
nb_amino_acids = 21
nb_species = 6


model_deepEvoSeq = DeepEvoSeqPositionGeneric(nb_species, nb_amino_acids, attention=use_attention, is_simple=is_simple)
# if is_simple:
#     if use_attention:
...
writer = SummaryWriter("runs_position_microgale/"+run_id+date_str)
with open("runs_position_microgale/"+run_id+date_str+'/specs.txt', 'w', encoding='utf-8') as f:    # save model specs and training hyperparameters
...
for epoch in range(num_epochs):
    ...
    for batch_x, batch_y in tqdm(train_loader):
        ...
        if is_simple:
            ...
            logits = model_deepEvoSeq(reps_all)
    ...
    top_5_keys = sorted(epoch_max_accs.keys(), reverse=True)[:5]
    epoch_max_accs = {k: epoch_max_accs[k] for k in top_5_keys}
    ...
    # all_data_to_save = all_data_to_save[-10:]    # keep only the last values because it's too heavy otherwise

data_best_5_epochs = [item for sublist in epoch_max_accs.values() for item in sublist]
np.save('runs_position_microgale/'+run_id+date_str+'/position_eval_values.npy', data_best_5_epochs)