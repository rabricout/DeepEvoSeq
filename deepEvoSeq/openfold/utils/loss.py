...
# RAPH: My loss that compares true sequence with predicted sequence on positions defined by mask_substitutions_ancestor
def seq_crossentropy(seq_pred, label_seq, anchor_seq, merged_mask, mask_substitutions_ancestor, seq_length_crop, residue_index, **kwargs,):
...
def metrics_computation_batch(cerberus_predictions, label_seq, ancestor_seq, anchor_seq, mouse_seq, human_seq, residue_index, mask_substitutions_ancestor, merged_mask, seq_length_crop, residue_index_rev, seq_length, msa_feat, folder_id, **kwargs,):
    subst_rate_pred, subst_pos_pred, seq_pred = cerberus_predictions
    batch_size = seq_pred.size()[0]
    all_metrics = []
    for i in range(batch_size):
        all_metrics.append(metrics_computation_single(seq_pred[i], label_seq[i], ancestor_seq[i], anchor_seq[i], mouse_seq[i], human_seq[i], residue_index[i], mask_substitutions_ancestor[i], merged_mask[i], seq_length_crop[i], residue_index_rev[i], seq_length[i], msa_feat[i], folder_id[i]))
    return all_metrics


def metrics_computation_single(seq_pred, label_seq, ancestor_seq, anchor_seq, mouse_seq, human_seq, residue_index, mask_substitutions_ancestor, merged_mask, seq_length_crop, residue_index_rev, seq_length, msa, folder_id):
    ...
    s_residue_index_f  = torch.masked_select(residue_index, merged_mask).tolist()
    ...
    sim_pred_truth = sum([s_pred_aa_f[i]==s_truth_f[i] for i in range(m_len)]) / m_len
    sim_pred_ancestor = sum([s_pred_aa_f[i]==s_ancestor_f[i] for i in range(m_len)]) / m_len
    sim_truth_ancestor = sum([s_truth_f[i]==s_ancestor_f[i] for i in range(m_len)]) / m_len
    
    # similarity between truth and anchor but separated between similar positions and substitutions
    nb_sim_positions    = sum([s_truth_f[i]==s_ancestor_f[i] for i in range(m_len)])
    nb_nonSim_positions = sum([s_truth_f[i]!=s_ancestor_f[i] for i in range(m_len)])

    if nb_sim_positions > 0:
        sim_pred_truth_sim = sum([s_pred_aa_f[i]==s_truth_f[i] and s_truth_f[i]==s_ancestor_f[i] for i in range(m_len)]) / nb_sim_positions
    else:
        sim_pred_truth_sim = [float('nan')]
    if nb_nonSim_positions > 0:
        sim_pred_truth_nonSim = sum([s_pred_aa_f[i]==s_truth_f[i] and s_truth_f[i]!=s_ancestor_f[i] for i in range(m_len)]) / nb_nonSim_positions
    else:
        sim_pred_truth_nonSim = [float('nan')]
    ...
    metrics["sim_pred_truth"] = sim_pred_truth
    metrics["sim_pred_ancestor"] = sim_pred_ancestor
    metrics["sim_truth_ancestor"] = sim_truth_ancestor
    metrics["diff_sim_pred_truth_sim_truth_ancestor"] = sim_pred_truth - sim_truth_ancestor
    metrics["subst_rate_pred_ancestor"] = 1 - sim_pred_ancestor
    if sim_truth_ancestor != 1:
        metrics["frac_improvment_compared_to_ancestor"] = (sim_pred_truth - sim_truth_ancestor) / (1-sim_truth_ancestor)
    if str(sim_pred_truth_sim) != 'nan':
        metrics["sim_pred_truth_sim"] = sim_pred_truth_sim
    if str(sim_pred_truth_nonSim) != 'nan':
        metrics["sim_pred_truth_nonSim"] = sim_pred_truth_nonSim

    metrics['residue_index'] = s_residue_index_f
    metrics['seq_length'] = int(seq_length.item())
    metrics['folder_id'] = int(folder_id.item())
    return metrics
