...
def accuracy(nature_values):
    ...
    ci = stats.t.interval(confidence=0.95, df=len(accs)-1, loc=np.mean(accs), scale=stats.sem(accs))
    return sum(accs)/len(accs), pos/tot, ci
...
def accuracy_matrix(nature_values):
    ...
    for i in range(len(matrix_accuracy)):
        matrix_accuracy[i,i] = float("nan")
...
plt.xlabel("% of most common substitutuions", fontsize=16)
...
plt.ylim(0, 1)
plt.show()
plt.savefig(output_path/'accuracy_by_most_common_substitutions.svg')
...
plt.xticks(tick_marks, tick_labels, fontsize=14)
plt.yticks(tick_marks, tick_labels, fontsize=14)
plt.ylabel("True label", fontsize=16)
plt.xlabel("Predicted label", fontsize=16)
plt.show()
plt.savefig(output_path/'confusion_matrix.svg')
...
plt.xlabel("Predicted label", fontsize=16)
plt.show()
plt.savefig(output_path/'accuracy_matrix.svg')


print('> computing accuracy')
mean_of_accuracies, mean_accuracy, confidence_interval = accuracy(nature_values)
print('mean_of_accuracies:', mean_of_accuracies)
print('confidence interval', confidence_interval, 'width', confidence_interval[1]-confidence_interval[0])
...
plt.xlabel("Substitution rate", fontsize=16)
plt.ylabel("Accuracy", fontsize=16)
plt.xticks(fontsize=14)
plt.yticks(fontsize=14)
plt.xscale('log')
plt.ylim(0, 1)
plt.show()
plt.savefig(output_path/'accuracy_by_subst_rate.svg')
...
plt.ylim(0.2, 0.4)
plt.show()
plt.savefig(output_path/'accuracy_by_position.svg')
