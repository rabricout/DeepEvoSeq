...
data_path_a1 = Path('DATA_mmseqs/DATA_APLO_clustered/A1_clustered')
species = ['APLO', 'HOMO', 'MOUSE', 'MUSCA', 'SCIURUS']
data_paths = {s: Path('DATA_mmseqs/DATA_APLO_clustered/'+s) for s in species}


print('> loading raw aplodontia data')
all_data = {}
for s in species:
    data_raw = {}
...
for k, f in tqdm(enumerate(files)):
    for i, record in enumerate(SeqIO.parse(data_path_a1/f, "fasta")):
        ...
        data_aplo[s_id] = (s_id, all_data['APLO'][s_id])
...
for c in train_clusters:
    for m_id in clusters[c]:
        ...
        train_aplo.append(data_aplo[m_id])
...
for c in eval_clusters:
    for m_id in clusters[c]:
        ...
        eval_aplo.append(data_aplo[m_id])


# train_aplo = [v for k, v in data_aplo.items() if k in train_clusters]
# eval_aplo  = [v for k, v in data_aplo.items() if k in eval_clusters]


print('> saving split data')
m_dir = 'DATA/DATA_APLO/'
...
with open(m_dir+"train_aplo.pkl", "wb") as f:
    pickle.dump(train_aplo, f)
with open(m_dir+"eval_aplo.pkl", "wb") as f:
    pickle.dump(eval_aplo, f)
