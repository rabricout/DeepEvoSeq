...
class BaselineDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        m_id = self.ids[idx]   # e.g., a tensor or numpy array of shape (L, D)
...
# Loading model
if model_id=='Blosum':
    model = BaselineBlosum(method='probs')
elif model_id=='LG':
    model = BaselineLG(method='argmax')
    ...
    model = BaselineESM2()
elif model_id=='PhyloBayesMPI':
    model = BaselinePhyloBayesMPI()
else:
    sys.exit()
...
for epoch in range(num_epochs):
    ...
    pos = 0
    tot = 0
    for (batch_x, batch_x_a1, batch_ids), (batch_y, batch_p) in tqdm(eval_loader):
        if model_id=='ESM2':
            subst = np.array(model.forward(batch_x[0], batch_p))
        elif model_id=='PhyloBayesMPI':
            subst = np.array(model.forward(batch_x[0], batch_p, batch_ids[0]))
        else:
    ...
    print(pos/tot)
os.makedirs('runs_nature/'+model_id, exist_ok=True)
np.save('runs_nature/'+model_id+'/nature_eval_values.npy', data_to_save)