...
def metrics_computation_batch(cerberus_predictions, label_seq, ancestor_seq, anchor_seq, mouse_seq, human_seq, residue_index, mask_substitutions_ancestor, merged_mask, seq_length_crop, residue_index_rev, seq_length, **kwargs,):
    subst_rate_pred, subst_pos_pred, seq_pred = cerberus_predictions
    batch_size = seq_pred.size()[0]
    all_metrics = []
    for i in range(batch_size):
        all_metrics.append(metrics_computation_single(seq_pred[i], label_seq[i], ancestor_seq[i], anchor_seq[i], mouse_seq[i], human_seq[i], residue_index[i], mask_substitutions_ancestor[i], merged_mask[i], seq_length_crop[i], residue_index_rev[i], seq_length[i]))
    return all_metrics


def metrics_computation_single(seq_pred, label_seq, ancestor_seq, anchor_seq, mouse_seq, human_seq, residue_index, mask_substitutions_ancestor, merged_mask, seq_length_crop, residue_index_rev, seq_length):
